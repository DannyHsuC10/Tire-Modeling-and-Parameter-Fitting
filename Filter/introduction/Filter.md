...
6. 改進方向
多斜率or邏輯加入 : 假設使用其他的TTC資料可以之後會碰到多種斜率問題，這個部分可以進行改善，用與範圍遮罩相同的手法加入通用多段斜率和斜率變化率。感善後可以大幅提升通用性，理論上只需要改 class 中的build_slope_mask和build_masks即可。把build_masks加入可以讀更多的 json 結構，然後裡面加入or邏輯改善。