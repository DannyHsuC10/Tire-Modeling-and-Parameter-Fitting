...
## 輪胎分析工具(`other`)
為了方便進行設計與分析另外有一些常用工具可以輪胎分析。要使用建議先了大概了解過輪胎在做什麼比較知道算出來的結果代表什麼。

1. 輪胎性能比較(`diff_tire_test`) : 比較不同輪胎的峰值力量，與
1. 輪胎剛性擬和(`Tire_Z_Rigidity`) : 輪胎z剛性計算，用於懸吊設計。(也可使用官方或TTC資料，但要記得選工作情況相近的)
1. 摩擦係數解出(`coefficient_of_friction`) : 車輛設計時評估輪胎極限與作用力量極限用
